...
var selector_pagina = document.getElementById('selector_pagina');
var imagen_credencial = document.getElementById('credencial');
var correo_alumno = document.getElementById('correo_alumno');
var nombre_alumno = document.getElementById('nombre_alumno');
var label_editar = document.getElementById('label_editar');
...
$(document).ready(function(){
  ...
  $('#search').click(async function(){
    mt=document.getElementById('entrada').value;
    

    asignarImagen(mt, foto);
    asignarImagenCredencial(mt, imagen_credencial);
    datos_alumno = await getData(mt);
    assign_data(datos_alumno);
  ...
  async function asign_colors(month,año,mat){ //asignar colores a las casillas del calendario dependiendo si hubo falta
    var date= new Date(año,month+1,0).getDate();
    var date2= new Date(año,month,1).getDay();
    let dias=parseInt(date,10);

    let diasAsistencia = await getMonthsData(mat);
    let split_numbers = diasAsistencia[month].split(',');
  ...
  async function stats(month,año,mat,currentDate){ //asignar colores a las casillas del calendario dependiendo si hubo falta
    ...
    if ((month==6 && (16<=dia && dia<=31))|| (month==7 && (1<=dia && dia<26))){//intervalo de vacaciones
      ...
      let flag = false; 
        ...
        while( flag == false ){
          ...
          let split_numbers = diasAsistencia[mes_inicio].split(',');
          let end_variable;
...
function asignarImagenCredencial(matricula, foto){
  const imagenUrl = `/id_matricula/${matricula}.png`;
  ...
  function assign_data(datos_alumno){
    ...
    for (let i=0;i<((datos_alumno.length)-1);i++){
      places[i].textContent=labels[i]+' '+datos_alumno[i];//datos del alumno
     }
  }

  function default_data(){
    selector_pagina.value = 'Seleccione';
